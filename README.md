...
本项目希望能帮到相关课题的同学。上云功能可以使用微信小程序开发工具来完成小程序的开发或者其他办法。
...
| 模块 | 型号 | 数量 | 参考购买渠道 |
|------|------|------|------------|
| 主控 | STM32F103C8T6 最小系统板 | 1 | 淘宝 |
| 烟雾传感器 | MQ-2 | 1 | 淘宝 |
| 空气质量传感器 | MQ-135 | 1 | 淘宝 |
| 温湿度传感器 | DHT11 | 1 | 淘宝 |
| OLED 显示屏（已焊好针脚） | SSD1306 0.96 寸 128x64 I2C | 1 | 淘宝 |
| WiFi 模块 | ESP8266-01S | 1 | 淘宝 |
| 继电器模块 | 5V 单路继电器（低电平触发） | 1 | 淘宝 |
| 有源蜂鸣器 | 3.3V / 5V | 1 | 淘宝 |
| 面包板830孔+400孔 + 杜邦线 | — | 若干 | 淘宝 |
| 面包板供电模块MB-102 | - | 1 | 淘宝 |

> MQ-2 和 MQ-135 出厂后需要**预热一段时间**才能稳定读数，首次上电建议先让设备持续运行一段时间再校准。
> 有条件可以使用焊板焊接，这样不会导致接触不良。也可以上PCB制板。以上电子器件均为模块。
